...
def get_latest_report():
    ...
    if "google-apps" in mime:
        ...
        import io
        buf = io.BytesIO()
        downloader = MediaIoBaseDownload(buf, service.files().get_media(fileId=file["id"]))
        done = False
...
def md_to_html(text):
    ...
    for line in lines:
        ...
        if line.startswith("### "):
            if in_table: html += "</table>\n"; in_table = False
            if in_list: html += "</ul>\n"; in_list = False
            html += "<h3>" + clean(line[4:]) + "</h3>\n"
        elif line.startswith("## "):
            if in_table: html += "</table>\n"; in_table = False
            if in_list: html += "</ul>\n"; in_list = False
            html += "<h2>" + clean(line[3:]) + "</h2>\n"
        elif line.startswith("# "):
            if in_table: html += "</table>\n"; in_table = False
            if in_list: html += "</ul>\n"; in_list = False
            html += "<h1>" + clean(line[2:]) + "</h1>\n"
        elif re.match(r"^[\s|:\-]+$", line) and "|" in line: continue
        elif line.startswith("|"):
            if not in_table: html += "<table>\n"; in_table = True
            cells = [c.strip() for c in line.split("|") if c.strip()]
            html += "<tr>" + "".join("<td>" + clean(c) + "</td>" for c in cells) + "</tr>\n"
        elif line.startswith("- ") or line.startswith("* "):
            if in_table: html += "</table>\n"; in_table = False
            if not in_list: html += "<ul>\n"; in_list = True
            html += "<li>" + clean(line[2:]) + "</li>\n"
...
def build_html(report_text):
    ...
    fecha = dias[today.weekday()] + " " + str(today.day) + " de " + meses[today.month-1] + " de " + str(today.year)
    ...
    summary = ""
    if summary_lines:
        items = "".join("<p>" + l + "</p>" for l in summary_lines)
        summary = '<div class="exec-summary"><div class="exec-label">Quick Read &mdash; ' + fecha + '</div>' + items + '</div>'
    parts = [
        ...
        '*{margin:0;padding:0;box-sizing:border-box;}body{font-family:"DM Sans",sans-serif;background:var(--warm);color:var(--ink);}',
        ...
        '.logo span{color:var(--gold);}.nav-date{font-size:.72rem;letter-spacing:.1em;text-transform:uppercase;color:rgba(255,255,255,.4);}',
        ...
        '.hero{background:#1A1814;padding:7rem 4rem 3rem;position:relative;overflow:hidden;}',
        ...
        '.exec-summary p{font-size:.9rem;color:rgba(255,255,255,.78);line-height:1.75;margin-bottom:.5rem;}.exec-summary strong{color:#fff;}',
        ...
        '.report h1:first-child{margin-top:0;}.report h2{font-family:"Cormorant Garamond",serif;font-size:1.25rem;font-weight:500;color:var(--gold);margin:2rem 0 .8rem;}',
        '.report h3{font-size:.88rem;font-weight:600;margin:1.5rem 0 .5rem;text-transform:uppercase;letter-spacing:.08em;}',
        '.report p{font-size:.88rem;color:var(--muted);margin-bottom:.8rem;line-height:1.8;}.report ul{margin:.5rem 0 1.2rem 1.8rem;}',
        ...
        '.report thead tr td{font-weight:600;background:#1A1814;color:#fff;padding:.7rem 1rem;}',
        '.report tbody td{padding:.6rem 1rem;border-bottom:1px solid var(--border);}.report tbody tr:hover td{background:var(--warm);}',
        '.report hr{border:none;border-top:1px solid var(--border);margin:2rem 0;}.report strong{color:#1A1814;font-weight:600;}',
        ...
        '.disc{background:#1A1814;padding:1.5rem 4rem;border-top:1px solid rgba(255,255,255,.05);}',
        ...
        '</style></head><body>',
        '<nav><a href="index.html" class="logo">Grupo <span>Estrategika</span></a>',
        '<span class="nav-date">' + fecha.upper() + '</span>',
        '<a href="index.html" class="nav-back">&#8592; Inicio</a></nav>',
        '<div class="hero">',
        '<span class="hero-tag">Reporte Diario de Mercados &middot; Exclusivo para Clientes</span>',
        '<h1>Daily Market Report</h1>',
        '<p class="hero-sub">' + fecha + ' &middot; Preparado por Grupo Estrategika</p></div>',
        '<div class="content">' + summary + '<div class="report">' + body + '</div></div>',
        '<div class="disc"><p>Reporte de caracter informativo elaborado exclusivamente para clientes de Grupo Estrategika. No constituye asesoria de inversion.</p></div>',
        '<footer><div class="footer-logo">Grupo <span>Estrategika</span></div>',
        '<div class="footer-copy">&copy; 2026 &middot; Exclusivo para clientes &middot; grupoestrategika.com</div></footer>',
        '<script>',
        'document.querySelectorAll(".report table").forEach(function(tbl){',
        'var rows=Array.from(tbl.querySelectorAll("tr"));',
        'if(rows.length>1){var thead=document.createElement("thead"),tbody=document.createElement("tbody");',
        'tbl.innerHTML="";thead.appendChild(rows[0]);rows.slice(1).forEach(function(r){tbody.appendChild(r);});',
        'tbl.appendChild(thead);tbl.appendChild(tbody);}',
        'tbl.querySelectorAll("td").forEach(function(td){var t=td.textContent.trim();',
        'if(/^\\+[\\d.]/.test(t))td.classList.add("positive");',
        'else if(/^-[\\d.]/.test(t))td.classList.add("negative");});});',
        '</script></body></html>'
    ]
    return "".join(parts)
...
def deploy(html):
    ...
    site_files = glob.glob(SITE_FOLDER + "/*.html")
...
if __name__ == "__main__":
    ...
    print(datetime.now().strftime("%A %d %B %Y - %H:%M UTC"))
    print("=" * 50)
    report, name = get_latest_report()
    if not report: sys.exit(1)
    html = build_html(report)
    print("HTML: " + str(len(html)//1024) + "KB")
