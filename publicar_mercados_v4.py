...
def get_latest_report():
    """Get today's report from Drive. Returns None if not today's report."""
    ...
    if modified != today:
        print(f"Most recent Drive report is from {modified}, not today ({today}). Skipping regeneration.")
        return None, None
    print(f"Using today's report: {file['name']}")
...
def md_to_html(text):
    ...
    def flush_table():
        ...
        for i, row in enumerate(table_rows):
            ...
            if i == header_idx:
                out += '<thead><tr>' + ''.join(f'<th>{clean(c)}</th>' for c in cells) + '</tr></thead>\n<tbody>\n'
            else:
    ...
    for line in lines:
        ...
        if line.startswith('### '):
            ...
            if in_list: html += '</ul>\n'; in_list = False
            in_table = True; table_rows.append(line)
        elif line.startswith('- ') or line.startswith('* ') or line.startswith('  - '):
            if in_table: html += flush_table(); table_rows = []; in_table = False
            if not in_list: html += '<ul>\n'; in_list = True
            html += f'<li>{clean(line.lstrip("- *").lstrip())}</li>\n'
...
.report thead tr th{font-weight:600;background:var(--ink);color:#fff;padding:0.7rem 1rem;text-align:left;}
...
<script>
document.querySelectorAll('.report tbody td').forEach(function(td){{
...
def deploy(files_dict):
    ...
    print("Desplegando con Wrangler...")
    dist = tempfile.mkdtemp()
    try:
        for filename, content in files_dict.items():
            if isinstance(content, str): content = content.encode('utf-8')
            with open(os.path.join(dist, filename), 'wb') as f: f.write(content)
            print(f"  + {filename}")
        env = os.environ.copy()
...
if __name__ == "__main__":
    print(f"Grupo Estrategika — {datetime.now().strftime('%Y-%m-%d %H:%M')} UTC")
    ...
    for fname in SITE_FILES:
        ...
        if os.path.exists(site_path):
            with open(site_path, 'rb') as f: files_dict[fname] = f.read()

    if report:
        # Today's report found in Drive — regenerate mercados.html
        print("Generating fresh mercados.html from Drive report")
        files_dict['mercados.html'] = build_html(report).encode('utf-8')
    else:
        # No today's report in Drive — use existing site/mercados.html
        mercados_path = os.path.join('site', 'mercados.html')
        if os.path.exists(mercados_path):
            print("Using existing site/mercados.html (no new Drive report today)")
            with open(mercados_path, 'rb') as f: files_dict['mercados.html'] = f.read()
        else:
            print("No mercados.html found")
            sys.exit(1)
